#!/usr/bin/env python3
"""
Generic code for product or data services using thread pools.
"""
import asyncio
...
class ThreadPoolExecutorService(BaseService):
    """
    This is the generic class for services that employ a "ThreadPoolExecutor"
    to concurrently process the requests.

    In order the derive a functional service the following methods must be
    overloaded:
        - post_request
        - process_request (no asnyc in contrast to AsyncioTaskService)
    ...
    async def schedule_request_processing(self, request):
        ...
        For this class it doesn't seem to make much sense that this method
        is async. However BaseService expects a async function hence we
        provide one.
        ...
        """
        self.logger.debug("Creating executor if non existing yet.")
        if not hasattr(self, "executor"):
            self.executor = ThreadPoolExecutor()
            self.logger.info(
                "Created ThreadPoolExecutor with %s workers.",
    ...
    def _process_request_wrapper(self, request):
        """
        ...
        try:
            result = self.process_request(input_data=request["input_data"])
    ...
    async def get_request_result(self, request_ID: UUID):
        ...
        try:
            ...
            while not request["future"].done():
                # NOTE: This line here is currently not captured in the tests.
                await asyncio.sleep(0.5)
            # Now that we know the result is ready, fetch it.
