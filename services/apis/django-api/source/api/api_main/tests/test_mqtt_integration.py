...
def mqtt_to_db_setup(request, django_db_setup, django_db_blocker):
    ...
    mtd = MqttToDb(
        ...
        n_mtd_write_threads_overload=1,
    )
...
class TestMqttToDb():
    ...
    def test_datapoint_map_created(self):
        ...
        dp1_topic = ("test_connector_8/messages/%s/value" %
                     str(test_datapoint_1.id))
        dp2_topic = ("test_connector_8/messages/%s/value" %
...
# This may rather belong in test models, and potentially test querysets?
# Well at least the part of these tests that check if certain methods are
# called after certain actions, like e.g. a datapoint is created.
#
# @pytest.fixture(scope='class')
...
class TestApiMqttIntegration():
    ...
    def test_get_instance_returns_instance(self):
        ...
        initialized_instance = ApiMqttIntegration(
    ...
    # def setup_message_catcher(self):
    #     """
    #     Setup a helper that can be used by wait_for_message below.
    #     """
    #     self.mqtt_client.userdata = []
    #
    #     def on_message(client, userdata, msg):
    ...
    #         Store the received message so we can test it's correctness later.
    #         """
    #         client.userdata.append(msg)
    #
    #     self.mqtt_client.subscribe("django_api/mqtt_to_db/rpc")
    #     self.mqtt_client.on_message = on_message
    #
    #     self.ami = ApiMqttIntegration(
    #         mqtt_client=self.fake_client,
    #     )
    #
    # def wait_for_message(self):
    #     """
    #     Wait up to one second for a message that should appear in userdata.
    #     """
    #     waited_seconds = 0
    #     while not self.mqtt_client.userdata:
    #         time.sleep(0.005)
    ...
    #         if waited_seconds >= 1:
    #             break
    #     return self.mqtt_client.userdata
    #
    ...
    #     """
    #     self.setup_message_catcher()
    #
    #     self.ami.trigger_update_topics_and_subscriptions()
    #
    #     received_msgs = self.wait_for_message()
    #     assert received_msgs != []
    #
    #     assert len(received_msgs) == 1
    #     rcp_call_msg = received_msgs[0]
    #
    #     actual_topic = rcp_call_msg.topic
    #     expected_topic = "django_api/mqtt_to_db/rpc"
    #     assert actual_topic == expected_topic
    #
    #     actual_payload = json.loads(rcp_call_msg.payload)
    #     expected_payload = {
    #         "method": "update_topics_and_subscriptions",
    #         "kwargs": {},
    #     }
    #     assert actual_payload == expected_payload
    #
    #     actual_retain_flag = rcp_call_msg.retain
    #     expected_retain_flag = False
    #     assert actual_retain_flag == expected_retain_flag
    #
    #     actual_qos_level = msg.qos
