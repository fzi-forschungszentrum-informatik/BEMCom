...
class SensorFlow(SFTemplate):
    ...
    def parse_raw_msg(self, raw_msg):
        ...
        """
        parsed_raw_message = json.loads(raw_msg["payload"]["raw_message"])

        try:
            dict_inv = parsed_raw_message["Body"]["Data"]["Inverters"]["1"]
            dict_site = parsed_raw_message["Body"]["Data"]["Site"]
            dict_comb = {**dict_inv, **dict_site}

        except KeyError:
            logger.error("Received unexpected content from Fronius API")
            raise
